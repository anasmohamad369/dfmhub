...
function statusWord(s) {
  return { pass: "Compliant", marginal: "Marginal — action required", fail: "Non-compliant — immediate action required" }[s] || "—";
}

/* ---------------- component ---------------- */
export default function DFMHubEarthLineApp() {
  const [tab, setTab] = useState("site");

  const [siteName, setSiteName] = useState("");
  const [siteLoc, setSiteLoc] = useState("");
  const [occIdx, setOccIdx] = useState(0);
  const [dimL, setDimL] = useState(30);
  const [dimW, setDimW] = useState(20);
  ...
  const [ngDirect, setNgDirect] = useState(5);
  const [ngTd, setNgTd] = useState(40);
  const [lpsClass, setLpsClass] = useState("none");

  const [electrodeType, setElectrodeType] = useState("Pipe / rod electrode");
  const [electrodeCount, setElectrodeCount] = useState(4);
  ...
  const data = useMemo(() => {
    ...
    const Ng = ngMode === "direct" ? parseFloat(ngDirect) || 0 : (parseFloat(ngTd) || 0) * 0.1;
  ...
  .el-brand-row{display:flex;align-items:center;gap:10px;margin:8px 0 4px 0;}
  .el-brand-mark{ width:32px;height:32px;flex-shrink:0;border-radius:8px;
    background:linear-gradient(145deg,var(--amber),#c9821c);
    display:flex;align-items:center;justify-content:center;
    font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:13px;color:#181205;letter-spacing:-.02em; }
  .el-brand h1{ font-family:'Space Grotesk',sans-serif; font-size:18px; margin:0; font-weight:700; letter-spacing:-.01em; line-height:1.15; }
  .el-brand h1 span{display:block;font-size:10.5px;font-weight:500;color:var(--muted);letter-spacing:.06em;margin-top:2px;}
  .el-brand p{ margin:8px 0 0 0; font-size:11.5px; color:var(--muted); line-height:1.4; }
  ...
  const Pill = ({ text, kind }) => <span className={`el-pill el-pill-${kind}`}>{text}</span>;
    ...
    <div className="el-root">
      ...
      <aside id="el-sidebar">
        <div className="el-brand">
          ...
          <div className="el-brand-row">
            <div className="el-brand-mark">DF</div>
            <h1>DFMHUB<span>EARTHLINE CONSOLE</span></h1>
          </div>
          <p>Earthing audit &amp; lightning protection risk assessment tool</p>
        </div>
        ...
        <div className="el-sidebar-foot">DFMHUB · Preliminary screening tool. Final protection design must be certified by a qualified electrical / LPS engineer per local code.</div>
      </aside>
      ...
      <main className="el-main">
        ...
        {tab === "report" && (
          <section>
            <div className="el-panel-head">
              <div className="eyebrow">Step 04 · DFMHUB EarthLine</div>
